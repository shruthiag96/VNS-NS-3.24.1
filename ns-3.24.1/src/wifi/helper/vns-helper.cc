...
NS_LOG_COMPONENT_DEFINE ("VNSHelper");
...
void
VNSHelper::SetRemoteStationManager (std::string type,
...
void
VNSHelper::SetStandard (enum WifiPhyStandard standard)
{
  m_standard = standard;
}

NetDeviceContainer VNSHelper::Install(const WifiPhyHelper &phyHelper, const WifiMacHelper &macHelper, NodeContainer c) const {
  NetDeviceContainer devices;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i){
	  Ptr<Node> node = *i;
	  Ptr<WifiNetDevice> device = CreateObject<WifiNetDevice> ();
	  device->SetNode(node); // IMPORTANT
...
NetDeviceContainer VNSHelper::Install (const WifiPhyHelper &phy, const WifiMacHelper &mac, Ptr<Node> node) const{
	return Install(phy, mac, NodeContainer (node));
}

NetDeviceContainer VNSHelper::Install (const WifiPhyHelper &phy, const WifiMacHelper &mac, std::string nodeName) const {
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return Install(phy, mac, NodeContainer(node));
}

void VNSHelper::EnableLogComponents (void) {
	  ...
	  LogComponentEnable ("VNSWifiChannel", LOG_LEVEL_ALL);
	  LogComponentEnable ("VNSWifiPhy", LOG_LEVEL_ALL);
}


int64_t
VNSHelper::AssignStreams (NetDeviceContainer c, int64_t stream)
{
