...
#include "vns-wifi-helper.h"
#include "ns3/error-rate-model.h"
#include "ns3/propagation-loss-model.h"
#include "ns3/propagation-delay-model.h"
#include "ns3/vns-wifi-channel.h"
#include "ns3/vns-wifi-phy.h"
...
NS_LOG_COMPONENT_DEFINE ("VNSWifiHelper");
...
VNSWifiChannelHelper::VNSWifiChannelHelper ()
{
	searchR = -1;
	debug = false;
}

VNSWifiChannelHelper::VNSWifiChannelHelper(double minX, double minY, double maxX, double maxY){
	this->minX = minX;
	this->minY = minY;
	this->maxX = maxX;
	this->maxY = maxY;
...
VNSWifiChannelHelper
VNSWifiChannelHelper::Default (void)
{
  VNSWifiChannelHelper helper;
...
void
VNSWifiChannelHelper::AddPropagationLoss (std::string type,
...
void
VNSWifiChannelHelper::SetPropagationDelay (std::string type,
                                            std::string n0, const AttributeValue &v0,
                                            std::string n1, const AttributeValue &v1,
                                            std::string n2, const AttributeValue &v2,
                                            std::string n3, const AttributeValue &v3,
...
Ptr<VNSWifiChannel>
VNSWifiChannelHelper::Create (void) const
{
  Ptr<VNSWifiChannel> channel = CreateObject<VNSWifiChannel> ();
  channel->setExtents(minX,minY,maxX,maxY);
  channel->setMaximumSearchDistance(searchR);
...
int64_t
VNSWifiChannelHelper::AssignStreams (Ptr<VNSWifiChannel> c, int64_t stream)
{
  return c->AssignStreams (stream);
}

VNSWifiPhyHelper::VNSWifiPhyHelper ()
  : m_channel (0),
    m_pcapDlt (PcapHelper::DLT_IEEE802_11)
{
  m_phy.SetTypeId ("ns3::VNSWifiPhy");
}

VNSWifiPhyHelper
VNSWifiPhyHelper::Default (void)
{
  VNSWifiPhyHelper helper;
  helper.SetErrorRateModel ("ns3::NistErrorRateModel");
  return helper;
}

void
VNSWifiPhyHelper::SetChannel (Ptr<VNSWifiChannel> channel)
{
  m_channel = channel;
}
void
VNSWifiPhyHelper::SetChannel (std::string channelName)
{
  Ptr<VNSWifiChannel> channel = Names::Find<VNSWifiChannel> (channelName);
  m_channel = channel;
}
void
VNSWifiPhyHelper::Set (std::string name, const AttributeValue &v)
{
  m_phy.Set (name, v);
}

void
VNSWifiPhyHelper::SetErrorRateModel (std::string name,
                                      std::string n0, const AttributeValue &v0,
                                      std::string n1, const AttributeValue &v1,
                                      std::string n2, const AttributeValue &v2,
                                      std::string n3, const AttributeValue &v3,
...
Ptr<WifiPhy>
VNSWifiPhyHelper::Create (Ptr<Node> node, Ptr<WifiNetDevice> device) const
{
	Ptr<VNSWifiPhy> phy = m_phy.Create<VNSWifiPhy> ();
...
void
VNSWifiPhyHelper::SetPcapDataLinkType (enum SupportedPcapDataLinkTypes dlt)
{
  switch (dlt)
    {
    case DLT_IEEE802_11:
    ...
    case DLT_IEEE802_11_RADIO:
      ...
      return;
    default:
      NS_ABORT_MSG ("VNSWifiPhyHelper::SetPcapFormat(): Unexpected format");
    }
}

void
VNSWifiPhyHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  //
...
void
VNSWifiPhyHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream,
  std::string prefix,
  Ptr<NetDevice> nd,
