...
#ifndef VNS_WIFI_HELPER_H
#define VNS_WIFI_HELPER_H
...
public:
	  ...
	  /**
	   * Create a channel helper with extents parameters.
	   */
	  VNSWifiChannelHelper(double minX, double minY, double maxX, double maxY);
	   ...
	   */
	  static VNSWifiChannelHelper Default (void);
	   ...
	   */
	  Ptr<VNSWifiChannel> Create (void) const;
	  ...
	  */
	  int64_t AssignStreams (Ptr<VNSWifiChannel> c, int64_t stream);
...
public:
   ...
   */
  static VNSWifiPhyHelper Default (void);
   ...
   */
  void SetChannel (Ptr<VNSWifiChannel> channel);
  /**
  ...
  /*
   * The following methods were defined by rjf
   */

  inline void updateStep(){ if(m_channel) m_channel->updateStep(); };

  inline QTree* getTree(){ return (m_channel?m_channel->getTree():0); };
